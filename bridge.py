...
def _key():
    ...
    if not k and os.path.exists("secret_key.txt"):
        k=open("secret_key.txt").read().strip()
    if not k and os.path.exists("sk.txt"):
        k=open("sk.txt").read().strip()
    if not k:
...
def _contracts():
    with open("contract_info.json") as f:
        j=json.load(f)
    s=_w3("source").eth.contract(address=Web3.to_checksum_address(j["source"]["address"].lower()), abi=j["source"]["abi"])
...
def scan_blocks():
    ...
    from_src=max(0, latest_src-500)
    from_dst=max(0, latest_dst-500)

    try:
        deposits=s.events.Deposit.create_filter(fromBlock=from_src, toBlock="latest").get_all_entries()
    except:
        deposits=[]
    for e in deposits:
    ...
    try:
        unwraps=d.events.Unwrap.create_filter(fromBlock=from_dst, toBlock="latest").get_all_entries()
    except:
        unwraps=[]
    for e in unwraps:
